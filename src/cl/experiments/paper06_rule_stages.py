...
def evaluate(model,rows,stage,seed,batch_size=128):
    ...
    selected=[rows[i] for i in np.linspace(0,len(rows)-1,min(512,len(rows)),dtype=int)]
...
def main(args):
    ...
    for seed in config["model_seeds"]:
        if args.evaluate_only:
            model=TinyTransformerLM(config["vocab_size"],config["sequence_length"],config["width"],config["depth"],config["heads"]);model.load_state_dict(torch.load(out/f"{stage}_seed{seed}.pt",map_location="cpu",weights_only=True));model=model.to(device).eval();loss=[]
        else:model,loss=train(config,stage,seed,device)
        r,l=evaluate(model,rows,stage,seed);raw+=r;layers+=l;losses+=loss
        if not args.evaluate_only:torch.save(model.to("cpu").state_dict(),out/f"{stage}_seed{seed}.pt")
        write_csv(out/f"{stage}_competence_raw.csv",raw);write_csv(out/f"{stage}_layer_raw.csv",layers)
    if losses:write_csv(out/f"{stage}_training_loss.csv",losses)
