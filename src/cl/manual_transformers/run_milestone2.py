...
EXPECTED = {task: {"sa_only": True, "ff_only": False, "sa_ff": True} for task in ("root_recurrence", "implication_recurrence")}
...
def _construction(task: str) -> str:
    semantics = "parent facts" if task == "root_recurrence" else "implication rules"
    domain = "all 6 orderings of three non-root nodes x all 4 starts (24 trajectories; root depths 0--3)" if task == "root_recurrence" else "all 24 orderings of four atoms x all 4 fact positions (96 trajectories; chain lengths 1--4)"
    return f"""# {task.replace('_', ' ').title()} construction

This is a deterministic fixed-weight representability witness; no weight is trained.  A single local transition circuit is called once per generated token.  Atomic `MAP_X_Y` tokens store {semantics} in separate key/value subspaces, while `STATE_X` tokens place the latest generated symbol in a query-only subspace.  The model has `L=1`, `H=1`, `d_model=15`, `d_head=5`, identity residuals, zero positions, finite causal softmax, no LayerNorm/dropout, and a zero FF update.
...
The exhaustive declared domain is {domain}.  `STOP` is an explicit terminal value.  SA-only and SA+FF must reproduce every complete trajectory; FF-only cannot read the contextual map and must fail the full-domain gate.  These controls are encoding-relative.  The witness establishes bounded tested-depth autoregressive reuse, not minimality, unbounded closure, robustness outside the domain, or SGD acquisition.
"""
...
def main(output: Path) -> None:
    ...
    tasks = {
        ...
        "implication_recurrence": (build_implication_variants, eval_implication),
    }
    ...
    for task, (builder, evaluator) in tasks.items():
        ...
        for topology, (model, vocab) in builder().items():
            ...
            summaries.append({
                ...
                "minimum_logit_margin": min(float(s["logit_margin"]) for s in step_rows),
                "claim_scope": "fixed_weight_bounded_autoregressive_representability_only",
            })
            architectures.append({
                "task": task, "topology": topology, "controller": "M1", "data_location": "context", "tool": "none",
    ...
    write_rows(output / "manual_witness_summary.csv", _read(output / "manual_witness_summary.csv")[:10] + summaries)
    write_rows(output / "manual_architectures.csv", _read(output / "manual_architectures.csv")[:10] + architectures)
    write_rows(output / "manual_component_necessity.csv", _read(output / "manual_component_necessity.csv")[:3] + necessity)
    ...
    for ax, task in zip(axes, tasks):
        ...
        for topology in ("sa_only", "ff_only", "sa_ff"):
            ...
            ax.plot(depths, values, marker="o", label=topology.replace("_", " "))
        ax.set_title(task.replace("_recurrence", "")); ax.set_xlabel("Generated transition steps")
        ax.set_xticks(depths); ax.set_ylim(-.04, 1.04); ax.grid(alpha=.25)
    axes[0].set_ylabel("Exact trajectory accuracy"); axes[1].legend(fontsize=8)
