...
The new Paper 0.5 nested-pattern bridge sharpens this boundary. In behaviorally competent target-preserving lexical extensions, embedded-short update donors are causally more substitutable than unrelated same-target and nonequivalent donors, and training creates a matched update-similarity advantage absent at initialization. However, the same bridge fails competence for target-changing refinement, override, and multi-level hierarchy. Paper 0.6 may therefore reuse the donor hierarchy, matched controls, and update/head-overlap metrics, but not cite lexical nesting as semantic abstraction evidence. A future semantic run must first learn the target-changing hierarchy and then show causal reuse beyond surface and same-target controls.
...
The competence failure changes the series-level conclusion. Representational geometry can exceed a permutation baseline even when held-out behavior is poor; semantic-looking geometry is therefore not sufficient evidence of abstraction. Attention-motif invariance is also absent, while sibling/cross-category replacement contrasts are weak. Paper 0.5's same-target nested bridge is stronger because its qualified strata are behaviorally realized and embedded-short replacement preserves an actually learned continuation. Its new length/depth control sharpens the boundary: target-preserving patterns remain competent through length 8, but necessary modular patterns fall from 0.852 at length 3 to 0.500 or below at lengths 4--8. The preregistered architecture sweep is therefore blocked, so neither paper claims that predictive order determines minimum depth. Paper 1 may use Paper 0.6 features only as frozen negative controls until a competent replication exists. No semantic feature from this run authorizes persistent learning.
