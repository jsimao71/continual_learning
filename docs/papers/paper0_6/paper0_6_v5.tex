...
\graphicspath{{results/v6/frontier_isancestor_cpu/}{results/v5/figures/}{results/v4/figures/}{results/v2/figures/}{docs/papers/paper0_6/results/v6/frontier_isancestor_cpu/}{docs/papers/paper0_6/results/v5/figures/}{docs/papers/paper0_6/results/v4/figures/}{docs/papers/paper0_6/results/v2/figures/}}
...
The experiments use synthetic symbolic relations, and even the independent sweeps are finite, right-censored probes. Checkpoints are locally reproducible but excluded from version control; all reported numeric claims are backed by tracked raw CSVs. The v4 configuration is \path{configs/paper06/predicate_v4.json}; the capacity design is \path{configs/paper06/capacity_v5.json}; and the v6 stress design is \path{configs/paper06/stress_v6.json}. Generator and model support are in \path{src/cl/semantic/predicates.py}, \path{src/cl/semantic/predicate_stress.py}, and \path{src/cl/common/model_adapter.py}. The v6 runner and analyzer are \path{paper06_stress_frontier.py} and \path{paper06_stress_analyze.py}. Its six official cells were completed on CPU after an incomplete MPS attempt repeatedly stalled during optimization; the incomplete MPS run is excluded from every reported statistic. Tracked manifests, raw evaluations, stable-cell summaries, and figures identify the clean CPU run and its exact sample counts.

\section{Conclusion}
A model can solve explicit taxonomic lookup without learning a predicate that extrapolates in depth. In the harder shallow-train/deep-test instrument, four constructive predicates remain incompetent across the tested capacity allocations. \texttt{isAncestor} has a movable finite boundary: optimization rescues diagnostic tiers, and both selected architectures exhaust the tested $D$, $d$, and $b$ ranges, yet extra depth gives no frontier advantage and loses the strict $N=256$ gate. Because the encoding admits direct membership lookup, this is not a depth-only law or general predicate closure, and no compositional scaling law is estimable from it. Local \texttt{isAncestor} causality likewise favors direct contextual membership over ancestor-by-ancestor iteration, whereas competent S3 provides controlled evidence that lower-order computation is partially reused within a higher-order rule. Capacity, optimization, extrapolation, and causal role invariance are separate requirements; only their conjunction supports a reusable semantic relation.
