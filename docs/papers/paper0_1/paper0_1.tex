...
The framework separates representability from acquisition; fixed-weight witnesses make that distinction concrete. We manually specify five micro-Transformer tasks with identity residuals, causal finite-softmax attention, ReLU FF blocks, zero positional embeddings, and no LayerNorm or dropout. No weight is trained or fitted. Every matrix and canonical forward trace is recorded, and each declared tiny legal domain is exhaustively enumerated.
...
Implication / context & recurrent SA-only & $(1,1,15)$ & 96/96 trajectories & FF 24/96; SA+FF 96/96 & one rule transition reused to length 4\\
\bottomrule
\end{tabularx}
...
For associative lookup, the desired scaled score is 16 and three other final-query scores are zero, giving finite probability $e^{16}/(e^{16}+3)=.999999662395$. Grandparent uses the same finite selector twice: layer one writes the parent into a dedicated state subspace and layer two uses that state to retrieve the grandparent. The failed one-layer control transports only one hop. The M1 witnesses instead reuse one fixed one-hop selector autoregressively: all 24 declared root trajectories through depth three and all 96 implication trajectories through length four are exact, with minimum per-step margin .99999910. FF-only cannot read either contextual map and fails the full-domain gates. These finite-depth constructions separate recurrence from adding layers, but do not prove arbitrary-depth closure; longer prompts add softmax leakage and leave the tested domain. They establish neither minimality, uniqueness, robustness beyond that domain, nor that SGD will acquire these weights or mechanisms. Full matrices and per-step forward traces are in the companion manual; exhaustive rows and matrix CSVs are under \path{results/manual_witnesses/}.
