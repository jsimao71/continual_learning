...
The framework separates representability from acquisition; fixed-weight witnesses make that distinction concrete. We manually specify three micro-Transformers with identity residuals, causal finite-softmax attention, ReLU FF blocks, zero positional embeddings, and no LayerNorm or dropout. No weight is trained or fitted. Every matrix and canonical forward trace is recorded, and each declared tiny legal domain is exhaustively enumerated.
...
\bottomrule
\end{tabularx}
\caption{Milestone-1 manually weighted witnesses. ``SA'' and ``FF'' columns are encoding-relative controls, not universal architectural lower bounds.}\label{tab:manual-witnesses}
\end{table}

For associative lookup, the desired scaled score is 16 and three other final-query scores are zero, giving finite probability $e^{16}/(e^{16}+3)=.999999662395$. Grandparent uses the same finite selector twice: layer one writes the parent into a dedicated state subspace and layer two uses that state to retrieve the grandparent. The failed one-layer control transports only one hop. These constructions establish that the specified architectures can implement the functions under atomic pair/edge encodings. They establish neither minimality, uniqueness, robustness beyond the legal domain, nor that SGD will acquire these weights or mechanisms. Full matrices and forward traces are in the companion manual; exhaustive rows and matrix CSVs are under \path{results/manual_witnesses/}.
