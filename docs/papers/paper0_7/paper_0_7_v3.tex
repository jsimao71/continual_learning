...
mechanism. P2/F2 and deeper stages remain unrun. Predictions, margins, per-template
tables, checkpoints, and hashes are tracked under \path{results/p1_f1_gate_v1}.

\section{Conclusion}
Paper 0.7 asks which exactly characterized computations a fixed Transformer can learn and execute, which resources move each finite frontier, and where one-shot execution fails so that iterative or symbolic alternatives merit controlled comparison. The measured P1/F1 checkpoint establishes reliable acquisition only at the local entrance to both ladders; the boundary itself remains to be measured. The fuzzy rule-like proposal remains intact as a downstream, competence-gated application. This ordering turns an earlier blocked semantic protocol into a coherent formal program without weakening its evidential standard.
\end{document}
