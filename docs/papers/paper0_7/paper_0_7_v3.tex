...
\subsection{Executable dataset and phase-plan checkpoint}
...
The current formal-data checkpoint is generated by \path{cl.experiments.paper07_formal_phase} from \path{configs/paper07/formal_v1.json}. Its manifest records the dataset hash, validation outcome, row count, planned-run count, and the explicit flag \texttt{model\_results=false}; the train, validation, and test JSONL files and phase-plan CSV are stored under \path{results/formal_v1}. This makes the boundary between validated experimental infrastructure and pending empirical evidence machine readable.

\subsection{Measured entrance gates: P1 and F1}
We next ran a narrow acquisition checkpoint using the same two-layer, width-32,
two-head causal Transformer at three initialization seeds. P1 is a balanced binary
test of one-step modus ponens with length-matched negative controls that independently
break the fact, rule left-hand side, or consequent/query match. F1 asks for the exact
bound symbol in an atomic variable--constant match. Its 12 targets are exactly balanced
within canonical, reversed, constant-distractor, and predicate-distractor serializations.
Train, validation, and test generators have disjoint raw namespaces; the model-facing
symbol slots are deliberately shared so the experiment measures acquisition of the
local relation rather than impossible prediction of unseen output embeddings. There
are 3,072 training, 768 validation, and 768 test examples per task. P1 was trained for
1,200 updates and F1 for 2,400. The preregistered local gate requires at least .95 test
accuracy overall and in every template for all three seeds, stricter than the broader
.80/.90 exploratory thresholds above.

\begin{table}[H]
\centering
\caption{Held-out exact accuracy for the measured local gates. ``Worst template'' is
the minimum within-seed test accuracy over the three P1 or four F1 strata.}
\begin{tabular}{lrrrr}
\toprule
Task & Seed 11 & Seed 23 & Seed 37 & Worst template\\
\midrule
P1: one-step modus ponens & 1.000 & 0.982 & 1.000 & 0.980\\
F1: atomic variable binding & 1.000 & 1.000 & 1.000 & 1.000\\
\bottomrule
\end{tabular}
\end{table}

Both three-seed gates pass. For P1 seed 23, the three test strata score .980,
.984, and .980; the other P1 seeds are perfect. Every F1 seed and template is
perfect, and F1 remains perfect when decoded by unrestricted vocabulary argmax rather
than only the declared 12-symbol output grammar. Chance is .50 for balanced P1 and
$1/12$ for F1. Thus the small fixed model reliably acquires these two local operations
under the stated controls, but does not locate a depth frontier or establish systematic
composition, canonical most-general unification, ordered execution, or a causal
mechanism. Predictions, margins, per-template
tables, checkpoints, and hashes are tracked under \path{results/p1_f1_gate_v1}.

\subsection{Second gate: P2 passes, F2 stops the ladder}
The next CPU run preserved the architecture, seeds, split sizes, and strict .95
overall-and-every-template rule. P2 separates modus tollens from contraposition as a
rule transformation using six balanced polarity/direction controls; it received 1,600
updates. F2 adds one unary functor and predicts an exact binding symbol or explicit
failure under canonical/reversed and functor/predicate-mismatch controls; it received
3,200 updates. Its exact engine recomputes every substitution or failure certificate.

\begin{table}[H]
\centering
\caption{P2/F2 held-out exact accuracy. The final column is the minimum test-template
accuracy over all seeds; the gate requires every seed and template to reach .95.}
\begin{tabular}{lrrrr}
\toprule
Task & Seed 11 & Seed 23 & Seed 37 & Worst template\\
\midrule
P2: modus tollens/contraposition & .993 & 1.000 & .999 & .969\\
F2: unary-functor binding/failure & 1.000 & .917 & .979 & .901\\
\bottomrule
\end{tabular}
\end{table}

P2 passes the three-seed gate. F2 fails: seed 23 scores .901--.927 across all four
test strata, so its .917 aggregate is not an isolated serialization failure. Within
that seed, binding cases score .948 but explicit structural failures only .885; errors
therefore concentrate on rejecting mismatched structures, although both case types
contribute. Grammar-constrained and unrestricted-vocabulary accuracies are identical.
Seeds 11 and 37 reach 1.000 and .979, respectively, making the finding seed instability
rather than uniform incapacity. The preregistered stopping rule blocks P3/F3 and deeper
stages: no frontier or systematic-unification claim follows. The run used CPU, taking
19.6--20.9 seconds per P2 model and 40.0--45.0 seconds per F2 model. Audits, predictions,
runtime, hashes, and template tables are tracked under \path{results/p2_f2_gate_v1}.

\section{Conclusion}
Paper 0.7 asks which exactly characterized computations a fixed Transformer can learn and execute, which resources move each finite frontier, and where one-shot execution fails so that iterative or symbolic alternatives merit controlled comparison. Local P1/F1 acquisition and P2 controls are stable, but F2 exposes a seed-unstable unary-structure boundary and stops deeper inference under the registered gate. This is not yet a capacity frontier: training-budget and architecture controls would be required to distinguish optimization from representational limits. The fuzzy rule-like proposal remains intact as a downstream, competence-gated application. This ordering turns an earlier blocked semantic protocol into a coherent formal program without weakening its evidential standard.
\end{document}
