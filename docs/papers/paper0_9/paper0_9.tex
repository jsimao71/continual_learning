...
Stage E changes the training boundary with a 72k shared $K\le3$ prefix followed by matched $K\le3$ continuation or $K=4$ examples. Exposure makes M3's final $K=4$ accuracy rise to .990 mean/.984 worst seed, but this is interpolation and one call covers only .25 of the proof; $K=6,8$ remain at or below .005. M4 does not acquire the trained depth: $K=4$ mean/worst is .010/.000, while $K=3$ falls to .870/.656 and $K=2$ worst seed falls to .922. Its strict frontier therefore contracts from two to one. The intervention teaches bounded neural completion to M3 but not repeatable local execution to M4, and introduces an acquisition tradeoff rather than closure.

\begin{figure}[H]\centering
\includegraphics[width=.62\linewidth]{results/learned_controller_v3/analysis/figures/stage_e_ktrain_frontiers.png}
\caption{Final Stage-E machine-specific frontiers. Training on $K=4$ does not extend the iterative M4 frontier and reduces shorter-depth stability; M3 remains limited by one-call proof coverage.}
\label{fig:stage-e}
\end{figure}
...
\section{Conclusion}
Paper 0.9 asks where systematicity comes from when the same learned local operator is embedded in a minimal iterative machine. Local implication is reliably acquired, but one-token, free-trace, fully supervised structured, one-call T1, and iterative T1 machines all stop at measured depth three in the initial matched regime. Exact continuation through four times the original optimization budget, depth/width and head controls, and matched-draw diversity through 96k examples leave M4's $K=4$ unacquired. Even direct $K=4$ exposure teaches only M3's bounded neural completion: M4 remains near zero at the trained boundary and becomes less stable below it. The harness changes available computation, but the controller has not learned a repeatable procedure. These are finite negative acquisition results, not impossibility or closure claims; they isolate procedure acquisition, rather than tool availability, as the present bottleneck.
\end{document}
