...
\section{Matched stress frontier and budget audit}
...
Architecture & Seed & 1x & 2x & 4x\\\midrule
L2/W64/H4 & 11 & .188 & .906 & 1.000\\
 & 23 & .250 & 1.000 & 1.000\\
 & 37 & 1.000 & 1.000 & 1.000\\
L8/W32/H4 & 11 & 1.000 & 1.000 & 1.000\\
 & 23 & .375 & 1.000 & 1.000\\
 & 37 & .312 & .750 & 1.000\\\bottomrule
\end{tabular}
\caption{Held-out order-two baseline accuracy under exact-prefix budget scaling. Factor slopes are interpreted only at 4x, where all six cells cross the 0.80 gate.}
\end{table}

At 4x both architectures have $p^*_{\max}=2$, $n_{\max}=24$, $k_{\max}=32$, and $s_{\max}=32$ over the tested grid. The latter three are right-censored lower bounds because every maximum cell remains competent. The ten three-seed failures among 58 architecture/contrast cells are precisely the five $p^*\ge3$ cells for each architecture. Machine-readable seed, aggregate, trajectory, loss, and frontier tables are under \path{results/stress_frontier/learned_v1}, \path{learned_v2_matched}, and \path{learned_v4_matched}.

An earlier diagnostic directory named \path{learned_v2} changed initialization and data-order seeds with budget. It is explicitly marked \path{INVALID_BUDGET_COMPARISON.md}, is excluded from every table and figure, and supports no claim.
...
PYTHONPATH=src python -m cl.experiments.paper05_stress_run \
  ...
  --output docs/papers/paper0_5/results/stress_frontier/learned_v4_matched
PYTHONPATH=src python -m cl.experiments.paper05_stress_analyze \
  --input docs/papers/paper0_5/results/stress_frontier/learned_v4_matched
